...
import { usePrimarySidebarStore } from '@stores/primary-sidebar';
import type { Mock } from 'vitest';

// Mock the stores
vi.mock('@stores/primary-sidebar');
vi.mock('@stores/app', () => ({
  useAppStore: () => ({
    currentView: 'books',
...
describe('PrimarySidebar', () => {
  ...
  beforeEach(() => {
    (usePrimarySidebarStore as unknown as Mock).mockImplementation(() => mockStore);
  });
  ...
  it('renders correctly', () => {
    render(<PrimarySidebar {...mockProps} />);
  ...
  //   expect(mockStore.setWidth).toHaveBeenCalledWith(200);
  ...
  it('updates isEmpty when items change', () => {
    const { rerender } = render(<PrimarySidebar {...mockProps} />);
    expect(mockStore.setIsEmpty).toHaveBeenCalledWith(false);

    rerender(<PrimarySidebar {...mockProps} items={[]} />);
